    ...
    internal class SpriteFrameCapabilityWindow : EditorWindow
    {
        ...
        public void CreateGUI()
        {
            ...
            m_WarningText = m_Warning.Q<TextElement>("warningText");
             if (m_Extension != null)
                 m_WarningText.text = string.Format(s_WarningText, m_Extension);
            UpdateUI();
            SpriteFrameModuleSettingsAsset.instance.SettingsChanged+= UpdateUI;
        }
        ...
        void UpdateUI()
        {
            if (m_Capability != null)
            {
                ...
                m_SpriteNameToggle?.SetValueWithoutNotify(!m_Capability.HasCapability(EEditCapability.EditSpriteName));
                m_SpriteNameToggle?.SetEnabled(m_Capability.originalData.HasCapability(EEditCapability.EditSpriteName) || allowOverride);

                m_PositionToggle?.SetValueWithoutNotify(!m_Capability.HasCapability(EEditCapability.EditSpriteRect));
                m_PositionToggle?.SetEnabled(m_Capability.originalData.HasCapability(EEditCapability.EditSpriteRect) || allowOverride);

                m_BorderToggle?.SetValueWithoutNotify(!m_Capability.HasCapability(EEditCapability.EditBorder));
                m_BorderToggle?.SetEnabled(m_Capability.originalData.HasCapability(EEditCapability.EditBorder) || allowOverride);

                m_PivotToggle?.SetValueWithoutNotify(!m_Capability.HasCapability(EEditCapability.EditPivot));
                m_PivotToggle?.SetEnabled(m_Capability.originalData.HasCapability(EEditCapability.EditPivot) || allowOverride);

                m_CreateToggle?.SetValueWithoutNotify(!m_Capability.HasCapability(EEditCapability.CreateAndDeleteSprite));
                m_CreateToggle?.SetEnabled(m_Capability.originalData.HasCapability(EEditCapability.CreateAndDeleteSprite) || allowOverride);
                UpdateWarningAndHelp();
            }
        }
        ...
        void UpdateWarningAndHelp()
        {
            if (m_Warning != null)
            {
                ...
                if (SpriteFrameModuleSettingsAsset.instance.allowSpriteFrameEditCapabilityOverride)
                {
