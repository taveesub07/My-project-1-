...
namespace UnityEditor.U2D.Sprites
{
    internal abstract partial class SpriteFrameModuleBase : SpriteEditorModuleModeSupportBase
    {
        ...
        const string k_LockToolTip = "Editing is disabled. Go to the Lock dropdown at the toolbar above to change this.";
        ...
        private void RemoveMainUI(VisualElement mainView)
        {
            ...
            SpriteFrameModuleSettingsAsset.instance.SettingsChanged -= PopulateSpriteFrameInspectorField;
        }
        ...
        protected void PopulateSpriteFrameInspectorField()
        {
            ...
            var enabled = m_CurrentEditEditCapability.HasCapability(EEditCapability.EditSpriteName);
            m_NameElement.SetEnabled(enabled && containsMultipleSprites);
            m_NameElement.tooltip = enabled ? "" : k_LockToolTip;

            enabled = m_CurrentEditEditCapability.HasCapability(EEditCapability.EditSpriteRect);
            m_PositionFieldContainer.SetEnabled(enabled);
            m_PositionFieldContainer.tooltip = enabled ? "" : k_LockToolTip;

            enabled = m_CurrentEditEditCapability.HasCapability(EEditCapability.EditBorder);
            m_BorderFieldContainer.SetEnabled(enabled);
            m_BorderFieldContainer.tooltip = enabled ? "" : k_LockToolTip;

            enabled = m_CurrentEditEditCapability.HasCapability(EEditCapability.EditPivot);
            m_PivotFieldContainer.SetEnabled(enabled);
            m_PivotFieldContainer.tooltip = enabled ? "" : k_LockToolTip;
        }
