    ...
    internal partial class SpriteFrameModule : SpriteFrameModuleBase
    {
        ...
        public void DoSpriteFrameMainGUI()
        {
            ...
            if (!spriteEditor.editingDisabled)
            {
                ...
                if (containsMultipleSprites && m_CurrentEditEditCapability.HasCapability(EEditCapability.CreateAndDeleteSprite))
                {
                    HandleCreate();
                    HandleDelete();
                    HandleDuplicate();
        ...
        public override void DoToolbarGUI(Rect toolbarRect)
        {
            using (new EditorGUI.DisabledScope(!containsMultipleSprites || spriteEditor.editingDisabled || m_TextureDataProvider.GetReadableTexture2D() == null || m_CurrentModuleMode?.GetType() != typeof(SpriteFrameEditorMode)))
            {
                ...
                var canCreateSprite = m_CurrentEditEditCapability.HasCapability(EEditCapability.CreateAndDeleteSprite);
                ...
                // Trim Button
                var canEditPosition = m_CurrentEditEditCapability.HasCapability(EEditCapability.EditSpriteRect);
        ...
        private void HandleRectCornerScalingHandles()
        {
            ...
            bool canEdit = m_CurrentEditEditCapability.HasCapability(EEditCapability.EditSpriteRect);
        ...
        {
            if (hasSelected && !MouseOnTopOfInspector() && m_CurrentEditEditCapability.HasCapability(EEditCapability.EditSpriteRect))
            {
