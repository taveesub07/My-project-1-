...
namespace UnityEditor.U2D.Sprites
{
    internal class SpriteUtilityWindow : EditorWindow
    {
        ...
        protected const float k_DefaultZoomPercentage = 0.9f;
        ...
        private static float cachedMinZoom = float.NaN;
        private static float cachedMaxZoom = float.NaN;
        private static float logMinZoom = float.NaN;
        private static float logMaxZoom = float.NaN;

        protected static float minZoom
        {
            get
            {
                float minZoomLevel = SpriteEditorWindowSettings.minZoomLevel;
                if (cachedMinZoom != minZoomLevel)
                {
                    cachedMinZoom = minZoomLevel;
                    logMinZoom = Mathf.Log(cachedMinZoom);
                }
            ...
            {
                cachedMinZoom = value;
                SpriteEditorWindowSettings.minZoomLevel = cachedMinZoom;
                logMinZoom = Mathf.Log(cachedMinZoom);
            }
        }

        protected static float maxZoom
        {
            get
            {
                float maxZoomLevel = SpriteEditorWindowSettings.maxZoomLevel;
                if (cachedMaxZoom != maxZoomLevel)
                {
                    cachedMaxZoom = maxZoomLevel;
                    logMaxZoom = Mathf.Log(cachedMaxZoom);
                }
            ...
            {
                cachedMaxZoom = value;
                SpriteEditorWindowSettings.maxZoomLevel = cachedMaxZoom;
                logMaxZoom = Mathf.Log(cachedMaxZoom);
            }
        }
        ...
        public float zoomLevel
        {
            ...
            set
            {
                if (m_Zoom <= 0f)
                    ...
                    m_Zoom = Mathf.Clamp(value, GetMinZoom(), GetMaxZoom());
            }
        }
        ...
        protected float GetFitToViewZoom()
        {
            if (m_Texture == null)
                return 0.0f;
            ...
            return Mathf.Min(m_TextureViewRect.width / m_Texture.width, m_TextureViewRect.height / m_Texture.height, maxZoom);
        }
        ...
        protected float GetMaxZoom()
        {
            ...
            if (defaultZoom <= 0.0f)
                return maxZoom;
            return defaultZoom * maxZoom;
        }

        protected float GetDefaultZoom()
        {
            return GetFitToViewZoom() * k_DefaultZoomPercentage;
        }
        ...
        protected void HandleZoom()
        {
            ...
            if (UnityEvent.current.type == EventType.ScrollWheel || (UnityEvent.current.type == EventType.MouseDrag && UnityEvent.current.alt && UnityEvent.current.button == 1))
            {
                ...
                // Clamp zoom
                float wantedZoom = zoomLevel * zoomMultiplier;

                float currentZoom = Mathf.Clamp(wantedZoom, GetMinZoom(), GetMaxZoom());
        ...
        protected Rect maxScrollRect
        {
            get
            {
                ...
                float halfWidth = m_Texture.width * .5f * zoom;
                float halfHeight = m_Texture.height * .5f * zoom;
        ...
        private static float ZoomScaleToSlider(float scale, float defaultZoom)
        {
            if (defaultZoom <= 0.0f)
                return 0.0f;
            float t = Mathf.Clamp(scale, defaultZoom * minZoom, defaultZoom * maxZoom);
            return (Mathf.Log(t / defaultZoom) - logMinZoom) / (logMaxZoom - logMinZoom) * 2.0f - 1.0f;
        }

        private static float ZoomSliderToScale(float slider, float defaultZoom)
        {
            if (defaultZoom <= 0.0f)
                return defaultZoom;
            float t = (Mathf.Clamp(slider, -1.0f, 1.0f) + 1.0f) * 0.5f;
            return defaultZoom * Mathf.Pow(minZoom, 1.0f - t) * Mathf.Pow(maxZoom, t);
        }
        ...
        protected Rect DoAlphaZoomToolbarGUI(Rect area)
        {
            ...
            drawArea.x -= drawArea.width;
            float defaultZoom = GetDefaultZoom();
            zoomLevel = ZoomSliderToScale(GUI.HorizontalSlider(drawArea, ZoomScaleToSlider(zoomLevel, defaultZoom), -1f, 1f, m_Styles.preSlider, m_Styles.preSliderThumb), defaultZoom);
