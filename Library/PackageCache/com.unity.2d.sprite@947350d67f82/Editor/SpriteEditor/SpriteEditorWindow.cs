...
namespace UnityEditor.U2D.Sprites
{
    ...
    public class SpriteDataProviderFactories
    {
        ...
        static HashSet<Type> s_FactorySupportedTypes;
        ...
        static void CacheDataProviders()
        {
            ...
            s_FactorySupportedTypes = s_Factories.Select(f => f.methodType).ToHashSet();
        ...
        /// <summary>
        /// Ensure that an AssetImporter type is supported by one of the <see cref="SpriteDataProviderFactory"/>.
        /// </summary>
        /// <param name="type">The type of AssetImporter</param>
        /// <returns>Is supported</returns>
        internal static bool IsTypeSupportedByFactories(Type type)
        {
            CacheDataProviders();
            return s_FactorySupportedTypes.Contains(type);
        }
    }
    ...
    internal class SpriteEditorWindow : SpriteUtilityWindow, ISpriteEditor, ISupportsOverlays
    {
        ...
        private class SpriteEditorWindowStyles
        {
            ...
            public static readonly GUIContent revertConfirmationDialogContent = EditorGUIUtility.TrTextContent("Are you sure you want to revert these changes?\nThis will permanently discard all edits you've made.\n(You can enable this dialog again in Preferences.)");
            public static readonly GUIContent applyConfirmationDialogTitle = EditorGUIUtility.TrTextContent("Apply Changes");
            public static readonly GUIContent applyConfirmationDialogContent = EditorGUIUtility.TrTextContent("Are you sure you want to apply these changes?\n(You can enable this dialog again in Preferences.)");
            public static readonly GUIContent revertLabel = EditorGUIUtility.TrTextContent("Revert");
            public static readonly GUIContent applyLabel = EditorGUIUtility.TrTextContent("Apply");
            public static readonly GUIContent keepEditingLabel = EditorGUIUtility.TrTextContent("Keep Editing");
            public static readonly string styleSheetPath = "Packages/com.unity.2d.sprite/Editor/UI/SpriteEditor/SpriteEditor.uss";
            public static readonly string helpBoxstyleSheetPath = "Packages/com.unity.2d.sprite/Editor/UI/SpriteEditor/SpriteEditorHelpBox.uss";
        ...
        private static readonly string k_RevertConfirmationOptOutKey = "SpriteEditorWindow.ShowRevertConfirmation";
        private static readonly string k_ApplyConfirmationOptOutKey = "SpriteEditorWindow.ShowApplyConfirmation";
        ...
        private bool SetupVisualElements()
        {
            ...
            if (styleSheet != null && rootVisualElement != null && rootVisualElement.styleSheetList != null)
            {
                ...
                m_HintPopup = new HelpBox();
                var helpboxStyleSheet = AssetDatabase.LoadAssetAtPath<StyleSheet>(SpriteEditorWindowStyles.helpBoxstyleSheetPath);
                m_HintPopup.styleSheets.Add(helpboxStyleSheet);
                m_HintPopup.AddToClassList("sprite-editor-hint-popup");
                m_HintPopup.messageType = HelpBoxMessageType.Warning;
                m_HintPopup.text = SpriteEditorWindowStyles.editingDisableMessageBecausePlaymodeLabel.text;
        ...
        void ShowEditorMessage(string message, HelpBoxMessageType messageType)
        {
            if(m_HintPopup == null)
                return;
            ...
            else if(m_HintPopup.text != message || m_HintPopup.style.display != DisplayStyle.Flex)
            {
                m_HintPopup.text = message;
                m_HintPopup.messageType = messageType;
        ...
        private void DoTextureAndModulesGUI()
        {
            ...
            if (!activeDataProviderSelected)
            {
                ...
                {
                    DoTextureGUI();
                    ShowEditorMessage(SpriteEditorWindowStyles.selectionNotEditableBySpriteEditor.text, HelpBoxMessageType.Info);
                }
                else
                {
                    ShowEditorMessage(SpriteEditorWindowStyles.noSelectionWarning.text, HelpBoxMessageType.Info);
                }
                return;
            }
            if (m_CurrentModule == null)
            {
                ShowEditorMessage(SpriteEditorWindowStyles.noModuleWarning.text, HelpBoxMessageType.Info);
                return;
            }
            ShowEditorMessage(null, HelpBoxMessageType.None);
            textureViewRect = new Rect(0f, 0f, m_MainViewIMGUIElement.layout.width - k_ScrollbarMargin, m_MainViewIMGUIElement.layout.height - k_ScrollbarMargin);
        ...
        private void DoApplyRevertGUI()
        {
            ...
            using (new EditorGUI.DisabledScope(!textureIsDirty))
            {
                if (GUILayout.Button(SpriteEditorWindowStyles.revertButtonLabel, EditorStyles.toolbarButton))
                {
                    ...
                    {
                        revert = EditorDialog.DisplayDecisionDialogWithOptOut(
                            SpriteEditorWindowStyles.revertConfirmationDialogTitle.text,
                            SpriteEditorWindowStyles.revertConfirmationDialogContent.text,
                            SpriteEditorWindowStyles.revertLabel.text,
                            SpriteEditorWindowStyles.keepEditingLabel.text,
                            DialogOptOutDecisionType.ForThisUser,
                            optOutKey: k_RevertConfirmationOptOutKey);

                        string optOutKey = k_OptOutPrefix + k_RevertConfirmationOptOutKey;
                        bool dontAskAgain = EditorPrefs.HasKey(optOutKey) && (DialogResult)EditorPrefs.GetInt(optOutKey) == DialogResult.DefaultAction;
                        if (dontAskAgain)
                            SpriteEditorWindowSettings.showRevertConfirmation = false;

                        EditorPrefs.DeleteKey(optOutKey);
                    }
                }
                if (GUILayout.Button(SpriteEditorWindowStyles.applyButtonLabel, EditorStyles.toolbarButton))
                {
                    ...
                    {
                        apply = EditorDialog.DisplayDecisionDialogWithOptOut(
                            SpriteEditorWindowStyles.applyConfirmationDialogTitle.text,
                            SpriteEditorWindowStyles.applyConfirmationDialogContent.text,
                            SpriteEditorWindowStyles.applyLabel.text,
                            SpriteEditorWindowStyles.keepEditingLabel.text,
                            DialogOptOutDecisionType.ForThisUser,
                            optOutKey: k_ApplyConfirmationOptOutKey);

                        string optOutKey = k_OptOutPrefix + k_ApplyConfirmationOptOutKey;
                        bool dontAskAgain = EditorPrefs.HasKey(optOutKey) && (DialogResult)EditorPrefs.GetInt(optOutKey) == DialogResult.DefaultAction;
                        if (dontAskAgain)
                            SpriteEditorWindowSettings.showApplyConfirmation = false;
        ...
        private void DoEditingDisabledMessage()
        {
            if (editingDisabled)
            {
                ...
                ShowEditorMessage(disableMessage, HelpBoxMessageType.Warning);
            }
        }
