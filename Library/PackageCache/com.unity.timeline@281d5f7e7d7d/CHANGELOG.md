...
- Removed the obsolete `TimelineProjectSettings.assetDefaultFramerate` field. Use `TimelineProjectSettings.defaultFrameRate` instead. Existing `TimelineSettings.asset` files migrate automatically via `FormerlySerializedAs`. (UUM-141989)

### Fixed

- Fixed `ArgumentOutOfRangeException` and `Invalid GUILayout state` errors thrown when clicking the key selection area at the position of a key from a previously selected property in the Timeline curves view. (UUM-143996)
- Fixed Timeline window toolbar buttons (Edit Mode, Show Markers, Sequence Switcher) appearing distorted by an unintended background image. (UUM-137722)
