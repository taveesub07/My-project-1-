...
namespace UnityEditor.Timeline
{
    abstract class TimelineTrackBaseGUI : TreeViewItem, IBounds
    {
        ...
        protected void DrawNameLabel(Rect rect, Color textColor, GUIStyle textStyle)
        {
            var defaultColor = SelectionManager.Contains(track) ? Color.white : textColor;
            ...
            rect.width = Math.Min(textStyle.CalcSize(GUIContent.Temp(track.name)).x, availableWidth);

            string newName;
            textStyle.clipping = TextClipping.Ellipsis;

            var controlId = track.GetObjectId().GetHashCode();
            ...
            newName = EditorGUI.DelayedTextField(rect, GUIContent.none, controlId, track.name, textStyle);
