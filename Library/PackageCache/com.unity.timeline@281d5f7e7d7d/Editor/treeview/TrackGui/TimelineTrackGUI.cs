...
namespace UnityEditor.Timeline
{
    class TimelineTrackGUI : TimelineGroupGUI, IClipCurveEditorOwner, IRowGUI
    {
        ...
        float DrawTrackIconKind(Rect rect, WindowState state)
        {
            ...
            const float iconSize = WindowConstants.trackHeaderButtonSize;
            const float iconMargin = WindowConstants.trackHeaderButtonPadding * 2;

            rect.x += iconMargin;
            rect.yMin += (rect.height - iconSize) / 2f;
            rect.width = iconSize;
            ...
            {
                m_ProblemIcon.image = DirectorStyles.warningIcon;
                m_ProblemIcon.tooltip = m_TrackDrawOptions.errorText;
                content = m_ProblemIcon;
            }
            else
            {
                content = GUIContent.Temp(m_TrackDrawData.m_TrackIcon, m_DefaultTrackIcon.tooltip);
            }
            ...
            if (CanDrawIcon(content))
            {
                ...
                if (!string.IsNullOrEmpty(content.tooltip))
                    GUI.Label(rect, GUIContent.Temp(string.Empty, content.tooltip));
            }

            return iconSize + iconMargin * 2;
        }
        ...
        void DrawTrackBinding(Rect rect)
        {
            ...
            DrawNameLabel(rect, m_Styles.customSkin.colorTrackFont, textStyle);
         }
